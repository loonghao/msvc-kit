...
| 4.5 | `msvc-kit = "0.1"` in every `Cargo.toml` sample | Current version is 0.2.17 | **Fixed** (both `api/library.md`) |
...
| 4.8 | Action input `host-arch` "empty = auto-detect" | The action maps an empty value to `x64` when building tool paths | **Fixed** (`guide/github-action.md`) |
| 4.9 | Action input `install-dir` default `$RUNNER_TEMP/msvc-kit` | `action.yml` default is `""`; the step resolves it to `$RUNNER_TEMP/msvc-kit` at run time | **Fixed** — documented as "empty = `$RUNNER_TEMP/msvc-kit`" |
